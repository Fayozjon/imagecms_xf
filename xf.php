 ...
 */
class Xf extends MY_Controller {
...
// Получаем данные страниц
        ...
        $config['base_url'] = site_url('xf/pages/' . http_build_query($search_data, '', '/'));
    ...
    public function group($group_id = 0) {
        ...
        $form_html = form_open('xf/group/' . (int) $group_id);
    ...
    public function create_filter_form($group_id, $by_fields = FALSE) {
        ...
        $numrows = $this->db->query('SELECT * FROM `content_fields`')->result_array();
    ...
    public function search_items($fields = array(), $type = 'page') {
        ...
        foreach ($fields as $key => $val) {
            ...
            if ($val != '' AND strstr($key,'_min')) {
            	$search_fields[] = str_replace('_min', '', $key);
            }
        }
        ...
        foreach ($query as $key => $field) {
            ...
            if (in_array($field['type'], array('select', 'checkgroup', 'radiogroup')))
            	...
            	elseif(is_array($fields[$name]) && strstr($fields[$name][0],'min_')){
					$tmp_min_val = str_replace('min_','',$fields[$name][0]);
					$tmp_max_val = (isset($fields[$name][1]) && !empty($fields[$name][1]))?(str_replace('max_','',$fields[$name][1])):9999999999;
            	
					$where[] = "(t_$name.field_name='$name' AND t_$name.data between '" . $this->db->escape_str($tmp_min_val) . "' and '" . $this->db->escape_str($tmp_max_val) . "')";
